...
let myLoading = (text, clearTime) => {
  let tips = text || 'Loading...'
  let html = "<i class='fa fa-spinner fa-pulse fa-3x fa-fw'></i><span class='sr-only'>Loading...</span>"
  let clearMyLoading = constructor(tips, html)
...
let myLoadingSuccess = (text, clearTime) => {
  let tips = text || 'Success'
  let html = "<i class='fa fa-check-circle-o fa-4x fa-fw' style='color: #00EE00'></i><span class='sr-only'>Success</span>"
  let clearMyLoading = myLoading(tips, html)
  if (!!clearTime && clearTime >= 1200) {
    setTimeout(() => {
      clearMyLoading()
    }, clearTime)
...
let myLoadingFail = (text, clearTime) => {
  let tips = text || 'Failed'
  let html = "<i class='fa fa-remove fa-4x fa-fw' style='color: #00EE00'></i><span class='sr-only'>Failed</span>"
  let clearMyLoading = myLoading(tips, html)
  if (!!clearTime && clearTime >= 1200) {
    setTimeout(() => {
      clearMyLoading()
